...
#line 1 "/Users/gp/Documents/terl/libly/hydride/generator/src/main/resources/libhydrogen/hydrogen.h" 1
...
int hydro_hash_init(hydro_hash_state *state, const char ctx[8],
...
void hydro_secretbox_probe_create(uint8_t probe[16], const uint8_t *c,
                                  size_t c_len, const char ctx[8],
                                  const uint8_t key[32]);

int hydro_secretbox_probe_verify(const uint8_t probe[16], const uint8_t *c,
                                 size_t c_len, const char ctx[8],
                                 const uint8_t key[32])
    ;
...
int hydro_sign_final_create(hydro_sign_state *state, uint8_t csig[64],
                            const uint8_t sk[64]);

int hydro_sign_final_verify(hydro_sign_state *state, const uint8_t csig[64],
                            const uint8_t pk[32])
    ;

int hydro_sign_create(uint8_t csig[64], const void *m_, size_t mlen,
                      const char    ctx[8],
                      const uint8_t sk[64]);

int hydro_sign_verify(const uint8_t csig[64], const void *m_, size_t mlen,
                      const char    ctx[8],
                      const uint8_t pk[32]) ;
...
int hydro_kx_n_1(hydro_kx_session_keypair *kp, uint8_t packet1[(32 + 16)],
                 const uint8_t psk[32],
                 const uint8_t peer_static_pk[32]);

int hydro_kx_n_2(hydro_kx_session_keypair *kp, const uint8_t packet1[(32 + 16)],
                 const uint8_t psk[32], const hydro_kx_keypair *static_kp);
...
int hydro_kx_kk_1(hydro_kx_state *state, uint8_t packet1[(32 + 16)],
                  const uint8_t           peer_static_pk[32],
                  const hydro_kx_keypair *static_kp);

int hydro_kx_kk_2(hydro_kx_session_keypair *kp, uint8_t packet2[(32 + 16)],
                  const uint8_t           packet1[(32 + 16)],
                  const uint8_t           peer_static_pk[32],
                  const hydro_kx_keypair *static_kp);

int hydro_kx_kk_3(hydro_kx_state *state, hydro_kx_session_keypair *kp,
                  const uint8_t           packet2[(32 + 16)],
                  const hydro_kx_keypair *static_kp);
...
int hydro_kx_xx_1(hydro_kx_state *state, uint8_t packet1[(32 + 16)],
                  const uint8_t psk[32]);

int hydro_kx_xx_2(hydro_kx_state *state, uint8_t packet2[(32 + 32 + 16 + 16)],
                  const uint8_t packet1[(32 + 16)],
                  const uint8_t psk[32], const hydro_kx_keypair *static_kp);

int hydro_kx_xx_3(hydro_kx_state *state, hydro_kx_session_keypair *kp,
                  uint8_t       packet3[(32 + 16 + 16)],
                  uint8_t       peer_static_pk[32],
                  const uint8_t packet2[(32 + 32 + 16 + 16)],
                  const uint8_t psk[32], const hydro_kx_keypair *static_kp);

int hydro_kx_xx_4(hydro_kx_state *state, hydro_kx_session_keypair *kp,
                  uint8_t       peer_static_pk[32],
                  const uint8_t packet3[(32 + 16 + 16)],
                  const uint8_t psk[32]);
...
int hydro_kx_nk_1(hydro_kx_state *state, uint8_t packet1[(32 + 16)],
                  const uint8_t psk[32],
                  const uint8_t peer_static_pk[32]);

int hydro_kx_nk_2(hydro_kx_session_keypair *kp, uint8_t packet2[(32 + 16)],
                  const uint8_t packet1[(32 + 16)],
                  const uint8_t psk[32], const hydro_kx_keypair *static_kp);
...
int hydro_pwhash_create(uint8_t stored[128], const char *passwd,
                        size_t passwd_len, const uint8_t master_key[32],
                        uint64_t opslimit, size_t memlimit, uint8_t threads);

int hydro_pwhash_verify(const uint8_t stored[128], const char *passwd,
                        size_t passwd_len, const uint8_t master_key[32],
...
int hydro_pwhash_derive_static_key(uint8_t *static_key, size_t static_key_len,
                                   const uint8_t stored[128],
                                   const char *passwd, size_t passwd_len,
                                   const char    ctx[8],
                                   const uint8_t master_key[32],
                                   uint64_t opslimit_max, size_t memlimit_max, uint8_t threads_max);

int hydro_pwhash_reencrypt(uint8_t       stored[128],
                           const uint8_t master_key[32],
                           const uint8_t new_master_key[32]);

int hydro_pwhash_upgrade(uint8_t       stored[128],
                         const uint8_t master_key[32], uint64_t opslimit,
