...
public final class LibraryLoader {
    ...
    /**
     * Loads the sodium library and registers the native methods of {@link Hydrogen}
     * using the specified loading mode.
     ...
     * of {@link Hydrogen}. The library will be loaded at most once.
     *
    ...
    public void loadLibrary(String libLocator) {
        ...
        synchronized (lock) {
            ...
            try {
                loader.register(Hydrogen.class, libLocator);
                loaded = true;
    ...
    /**
     * Returns the absolute path to sodium library inside JAR (beginning with '/'), e.g. /linux/libhydrogen.so.
     */
    private static String getSodiumPathInResources() {
        boolean is64Bit = Native.POINTER_SIZE == 8;
        if (Platform.isWindows()) {
            if (is64Bit) {
                return getPath("windows64", "libhydrogen.dll");
            } else {
        ...
        if (Platform.isLinux()) {
            if (is64Bit) {
                ...
                return getPath("linux", "libhydrogen.so");
            }
        }
        if (Platform.isMac()) {
